...
export interface PulumiAfterDeployContext extends PulumiContext {
     ...
     */
    stacksFailed: StackDeployFailure[];
}
    ...
    return class DeployCommand extends Command {
        ...
        override async run() {
                    ...
                    const stackPromise = (async () => {
                        ...
                        .then(async () => {
                            ...
                            if (deploying.length > 0) {
                                const maxShow = 5;
                                const shown = deploying
                                    .slice(0, maxShow)
                                    .map(s => chalk.yellow(s.stackName))
                                    .join(', ');
                                const suffix =
                                    deploying.length > maxShow ? ` + ${deploying.length - maxShow} more` : '';
                                stackResolved.logger.info(`🔄 Deploying: ${shown}${suffix}`);
                            }

                            const remaining = stacks.filter(s => stacksLeft.has(s) && !stacksDeploying.has(s));
                            if (remaining.length > 0) {
                                const maxShow = 5;
                                const shown = remaining
                                    .slice(0, maxShow)
                                    .map(s => chalk.green(s.stackName))
                                    .join(', ');
                                const suffix =
                                    remaining.length > maxShow ? ` + ${remaining.length - maxShow} more` : '';
            ...
            await options.afterDeploy?.({
                ...
                stacksFailed: [...stacksFailed.entries()].map(([stack, error]) => ({ stack, error })),
            });
